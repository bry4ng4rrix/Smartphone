    ...
    return Scaffold(
      body: switch (async) {
        AsyncData(:final value) => value.isEmpty
            ? const EmptyState(message: 'Aucune référence. Ajoutez-en une.', icon: Icons.style_outlined)
            : RefreshIndicator(
                onRefresh: () => ref.read(referencesProvider.notifier).refresh(),
                child: ListView.builder(
                  padding: const EdgeInsets.all(12),
                  itemCount: value.length,
                  itemBuilder: (context, i) => _ReferenceTile(reference: value[i]),
                ),
              ),
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    return Card(
      ...
      child: ExpansionTile(
        ...
        subtitle: Text('${reference.categoryName} / ${reference.typeName} — ${_ar(reference.prixVente)}'),
        trailing: IconButton(
...
Future<String?> _promptText(BuildContext context, String title, TextEditingController controller, {String? hint}) {
