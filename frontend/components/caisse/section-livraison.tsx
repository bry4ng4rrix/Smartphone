...
const ETAT = {
  benefice: { label: 'Bénéfice livraison', cls: 'text-emerald-600 dark:text-emerald-400' },
  perte: { label: 'Perte livraison', cls: 'text-red-600 dark:text-red-400' },
  equilibre: { label: 'Équilibre', cls: 'text-muted-foreground' },
};
...
function Bloc({ titre, s }: { titre: string; s: StatsLivraison }) {
  ...
  return (
    <div className="rounded-lg border p-3 space-y-1.5">
      ...
      <div className="flex justify-between text-sm"><span className="text-muted-foreground">Livraison facturée</span><span className="tabular-nums">{fmtAr(s.facturee)}</span></div>
      <div className="flex justify-between text-sm"><span className="text-muted-foreground">Frais agences</span><span className="tabular-nums">{fmtAr(s.agence)}</span></div>
      <div className="flex justify-between text-sm font-semibold border-t pt-1.5">
        <span>{etat.label}</span>
        <span className={`tabular-nums ${etat.cls}`}>{Number(s.resultat) > 0 ? '+' : ''}{fmtAr(s.resultat)}</span>
      </div>
    </div>
  );
}
  ...
  return (
    <Card>
      <CardHeader className="pb-3">
        <CardTitle className="flex items-center gap-2 text-base"><Truck className="h-4 w-4" /> Résultats livraison</CardTitle>
      ...
      <CardContent>
        {loading || !data ? (
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-3">{[0, 1, 2, 3].map((i) => <Skeleton key={i} className="h-28" />)}</div>
        ) : (
