...
class OrderItemPublicSerializer(serializers.ModelSerializer):
    """Vue restreinte — préparateur/livreur : pas de prix (§4, §7.2, §7.3 README)."""
...
class OrderGerantSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Order
        fields = [
            "id", "numero", "date_commande", "client_nom", "telephone", "livraison_zone",
            "frais_livraison", "total_a_payer", "note", "statut_courant", "items",
...
class OrderPreparateurSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Order
        fields = ["id", "numero", "client_nom", "livraison_zone", "statut_courant", "items", "created_at"]
        read_only_fields = fields
...
class OrderLivreurSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Order
        fields = [
            "id", "numero", "client_nom", "telephone", "livraison_zone",
