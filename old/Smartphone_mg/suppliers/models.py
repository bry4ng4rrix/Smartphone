...
class SupplierOrder(models.Model):
    """Commande fournisseur (§7.6 README) — permet de calculer le coût de
    revient réel : marchandise + fret/import + douane + pub Meta Ads."""
    ...
    def generate_numero(self):
        ...
        prefix = f"SUP-{today:%Y%m%d}-"
        last = SupplierOrder.objects.filter(numero__startswith=prefix).order_by("-numero").first()
