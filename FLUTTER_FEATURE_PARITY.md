...
Écran Flutter : `features/dashboard/dashboard_screen.dart`  
Fonctionnalités à garantir : 43
...
Écran Flutter : `features/reports/reports_screen.dart`  
Fonctionnalités à garantir : 53
...
### `/* (RootLayout — enveloppe TOUTES les pages, y compris /login, /register, /forgo`  —  PARTIAL
...
### `/(app)/* — shell applicatif protege (couvre /dashboard, /orders, /pickup, /bilan`  —  PARTIAL
...
### `(aucune route) — components/ui/sidebar.tsx : kit de primitives shadcn/ui NON UTI`  —  PARTIAL
...
### `(composant partagé) <ConfirmDeleteDialog /> — modal de suppression avec ré-authe`  —  PARTIAL
...
### `(composant partagé) <AIAnalysis /> — carte « Analyse IA Stratégique ». Monté en `  —  PARTIAL
...
### `(hook partagé) lib/hooks/useDebouncedValue.ts — valeur temporisée pour les reche`  —  PARTIAL
...
### `djangoClient.passwordResetRequests — moderation des demandes de reinitialisation`  —  PARTIAL
...
### `djangoClient.catalog.references — References produit (fiche article) + import/ex`  —  PARTIAL
...
- [ ] Boutons de statut en bas de la fiche, sous l'historique détaillé ; « Modifier » seul en en-tête  `MISSING`
- [ ] La fiche reste ouverte jusqu'au statut terminal : après chaque transition elle se recharge et propose l'étape suivante  `MISSING`
- [ ] Confirmation intégrée (note + photo) sans fermer la fiche  `MISSING`
- [x] Photo de préparation : boutons « Prendre une photo » / « Choisir un fichier » + « Retirer »  `IMPLEMENTED`
- [ ] Assigner un préparateur / un livreur SANS faire avancer le statut  `PARTIAL`
- [ ] Boutons d'assignation proposés seulement tant que le poste est vacant ; livreur assignable dès que le préparateur l'est  `MISSING`
- [ ] Corriger l'état d'une commande close (Livré <-> Retour), gérant, avec remise en stock  `MISSING`
- [ ] Mot à retaper avant Retour (RETOUR) et Annuler (ANNULER)  `MISSING`
- [ ] Livraison partielle : oui/non pour 1 article, cases à cocher au-delà ; décoché = rapporté en stock et retiré du total ; rien = Retour  `MISSING`
- [ ] Second numéro de téléphone (création, modification, détail, colonne livreur cliquable)  `MISSING`
- [ ] Modifier reste disponible jusqu'à la livraison : zone, adresse, paiement, note livreur (frais et total recalculés)  `PARTIAL`
- [ ] Partager la commande (résumé + photo) au livreur assigné dans la messagerie  `MISSING`
- [ ] Ligne « Livraison prévue le / Livrée le » et métadonnées d'article étiquetées (Type, Sous-type, Marque, Quantité)  `PARTIAL`

### COMMANDES — /orders — listes par rôle

- [x] Tri : la plus récemment créée en haut, pour les 3 rôles  `IMPLEMENTED`
- [x] Livreur : tournée filtrée sur la fenêtre d'affichage (jour J, et 5 h avant) ; bouton masqué avant minuit  `IMPLEMENTED`
- [x] Livreur : commandes Nouvelle assignées visibles dans son planning  `IMPLEMENTED`
- [ ] Livreur : filtre statut réduit à « À récupérer » / « Livrées »  `PARTIAL`
- [x] Préparateur : fenêtre aujourd'hui + demain à partir de 19h00  `IMPLEMENTED`
- [x] Filtres de date par défaut au jour J (Antananarivo) et « Réinitialiser » qui y ramène  `IMPLEMENTED`

### COMMANDES — /bilan

- [ ] Gérant : liste des livreurs avec totaux du jour (livraisons, retours, produits, frais, encaissé), ligne Total, clic = détail  `MISSING`
- [ ] Gérant : sélecteur de jour  `MISSING`
- [x] Commande payée d'avance = 0 Ar encaissé, ligne « Dont payé d'avance »  `IMPLEMENTED`
- [ ] Dépenses du livreur : déclaration (type ou libre, montant, quantité, motif), liste avec statut, retrait tant qu'en attente  `MISSING`
- [ ] Gérant : accepter / rejeter chaque dépense ; colonne « Dépenses » et « n à valider » dans la vue d'ensemble  `MISSING`
- [ ] Ticket : « DÉPENSES VALIDÉES » et « NET À REMETTRE »  `MISSING`

### PILOTAGE — /reports

- [ ] Page refaite sur GET /api/orders/reports/ : CA, marge, dépenses, résultat, frais, dépenses livreurs, retours, taux  `MISSING`
- [ ] Graphique recettes / dépenses / écart par jour  `MISSING`
- [ ] Mouvements de stock par jour + tableau « Activité par jour » avec part en %  `MISSING`
- [ ] Produits les plus / les moins vendus  `MISSING`
- [ ] Performance des livreurs (assignées, livrées, retours, réussite, encaissé, frais, dépenses)  `MISSING`
- [ ] Commandes préparées par préparateur, détail par jour  `MISSING`
- [ ] Sélecteur 7 / 30 / 90 jours et bornes libres  `MISSING`
- [x] Vente à crédit supprimée  `IMPLEMENTED`

### COMMUNICATION — /chats

- [ ] Envoi d'image (fichier ou appareil photo) via POST /users/chat/upload/, affichage dans la bulle  `MISSING`
- [ ] Salon Général retiré : conversations directes uniquement  `MISSING`
- [ ] Badge de non-lus par contact  `MISSING`
- [ ] Liste triée : non-lus d'abord, puis conversation la plus récente  `MISSING`
- [ ] En-tête de page retiré ; liste des contacts défilante  `PARTIAL`

### COMMUNICATION — Navigation

- [ ] Badge de messages non lus sur l'entrée « Chats » du menu (GET /users/chat/unread-count/, toutes les 30 s et à chaque page)  `MISSING`

### ADMINISTRATION — /settings

- [ ] Carte « Dépenses des livreurs » : CRUD des types (nom, montant, à l'unité, actif, modifier, supprimer)  `MISSING`

### COMPOSANTS PARTAGÉS & SOCLE — Assistant

- [ ] Bulle flottante en bas à droite : questions sur le guide de l'app (POST /api/ai/assistant, mode guide)  `MISSING`
- [ ] Gérant : « Générer le rapport du mois » (mode rapport)  `MISSING`

### COMPOSANTS PARTAGÉS & SOCLE — Socle

- [x] Modèle de rôles admin / magasin / employer + drapeaux (core/permissions.dart)  `IMPLEMENTED`
- [x] Menu identique au sidebar web + gardes de route  `IMPLEMENTED`
- [x] Fuseau Indian/Antananarivo pour toutes les dates et le jour J  `IMPLEMENTED`
...
Fonctionnalités ajoutées depuis l'audit : 46 (MISSING 29 · PARTIAL 5 · IMPLEMENTED 12)

**Total de cases à cocher : 2196.**
