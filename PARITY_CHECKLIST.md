...
Checklist de parite fonctionnelle **Next.js -> Flutter**.
Chaque ligne provient de l'audit exhaustif (voir `FLUTTER_MIGRATION.md`).

Statuts : `DONE` (porte et verifie) · `PARTIAL` (partiellement porte) · `MISSING` (absent de Flutter) · `BLOCKED` (impossible, raison indiquee).
...
| `/(app)/* — shell applicatif protege (couvre /dashboard, /ord` | lib/widgets/navigation_shell.dart | 13 | PARTIAL |
| `/(app)/* — Sidebar de navigation (rendue sur toutes les rout` | lib/core/nav_items.dart + lib/widgets/navigation_shell.dart | 49 | PARTIAL |
| `/(app)/* — TopBar (barre superieure, rendue sur toutes les r` | lib/widgets/topbar.dart | 40 | PARTIAL |
| `* — GlobalErrorBoundary (composant transverse, actuellement ` | composant / couche partagee — a porter | 16 | PARTIAL |
| `* — ThemeProvider (wrapper de theme, monte dans app/layout.t` | composant / couche partagee — a porter | 6 | PARTIAL |
| `(aucune route) — components/ui/sidebar.tsx : kit de primitiv` | composant / couche partagee — a porter | 48 | PARTIAL |
| `(composant partagé) <ImageUpload /> — zone de dépôt + prévis` | composant / couche partagee — a porter | 48 | PARTIAL |
| `(composant partagé) <ProductImageGallery /> — galerie photos` | composant / couche partagee — a porter | 41 | PARTIAL |
| `(composant partagé) <ConfirmDeleteDialog /> — modal de suppr` | composant / couche partagee — a porter | 38 | PARTIAL |
| `(composant partagé) <AIAnalysis /> — carte « Analyse IA Stra` | composant / couche partagee — a porter | 41 | PARTIAL |
| `POST /api/ai/analyze — route API Next.js (Route Handler)` | composant / couche partagee — a porter | 23 | PARTIAL |
| `POST /api/ai/check-duplicates — route API Next.js (Route Han` | composant / couche partagee — a porter | 26 | PARTIAL |
| `(module partagé) lib/image-service.ts — service de gestion d` | composant / couche partagee — a porter | 18 | PARTIAL |
| `(module partagé) lib/qrcode-generator.ts — génération et lec` | composant / couche partagee — a porter | 15 | PARTIAL |
| `(hook partagé) lib/hooks/useDeliveryZones.ts — zones de livr` | composant / couche partagee — a porter | 20 | PARTIAL |
| `(hook partagé) lib/hooks/useDebouncedValue.ts — valeur tempo` | composant / couche partagee — a porter | 14 | PARTIAL |
| `(couche transport) DjangoAPIClient — coeur HTTP/JWT` | composant / couche partagee — a porter | 36 | PARTIAL |
| `djangoClient.auth — authentification, inscription, mot de pa` | composant / couche partagee — a porter | 25 | PARTIAL |
| `djangoClient.passwordResetRequests — moderation des demandes` | composant / couche partagee — a porter | 5 | PARTIAL |
| `djangoClient.catalog.categories — Categories du catalogue (§` | composant / couche partagee — a porter | 6 | PARTIAL |
| `djangoClient.catalog.types — Sous-types rattaches a une cate` | composant / couche partagee — a porter | 7 | PARTIAL |
| `djangoClient.catalog.brands — Marques` | composant / couche partagee — a porter | 5 | PARTIAL |
| `djangoClient.catalog.colors — Couleurs (referentiel des vari` | composant / couche partagee — a porter | 6 | PARTIAL |
| `djangoClient.catalog.references — References produit (fiche ` | composant / couche partagee — a porter | 21 | PARTIAL |
| `djangoClient.catalog.importBatches — Annulation d'un import ` | composant / couche partagee — a porter | 4 | PARTIAL |
| `djangoClient.catalog.variants — Variantes (couleur) et ajust` | composant / couche partagee — a porter | 9 | PARTIAL |
| `djangoClient.orders — Module Commandes (coeur metier, §5-§7)` | composant / couche partagee — a porter | 41 | PARTIAL |
| `djangoClient.zones — Zones de livraison configurables (Param` | composant / couche partagee — a porter | 12 | PARTIAL |
| `djangoClient.movements — Historique des mouvements de stock ` | composant / couche partagee — a porter | 16 | PARTIAL |
| `djangoClient.products — Service de compatibilite (lecture se` | composant / couche partagee — a porter | 11 | PARTIAL |
...
| `djangoClient.backup — Sauvegarde / restauration (admin uniqu` | composant / couche partagee — a porter | 5 | PARTIAL |
| `djangoClient.chat — Messagerie interne` | composant / couche partagee — a porter | 6 | PARTIAL |
| `lib/types.ts — Modeles TypeScript partages` | composant / couche partagee — a porter | 14 | PARTIAL |
| `lib/validation.ts — Schemas Zod (heritage, non branches sur ` | composant / couche partagee — a porter | 11 | PARTIAL |
| `lib/timezone.ts — Fuseau metier Indian/Antananarivo (regle d` | composant / couche partagee — a porter | 13 | PARTIAL |
| `lib/utils.ts — Helper de classes CSS` | composant / couche partagee — a porter | 2 | PARTIAL |

**Total : 2150 elements a garantir en parite.**

## Socle transverse

- [ ] Modele de roles complet : `admin` / `magasin` / `employer` + sous-role `PREPARATEUR`/`LIVREUR`, et les flags derives `isAdmin`, `isSuperAdmin`, `isGerant`, `isCompanyOwner`
- [ ] Gating de navigation par role identique au sidebar web (`adminOnly`, `superAdminOnly`, `livreurOnly`, `hidePreparateur`, `hideLivreur`)
- [ ] Gardes de route go_router refusant l'acces direct a une route interdite (aujourd'hui seul le menu filtre)
- [ ] Authentification : login, logout, refresh de token, session persistante, expiration
- [ ] Inscription, mot de passe oublie, reinitialisation, verification e-mail, compte en attente d'approbation
- [ ] Notifications temps reel (WebSocket) et rafraichissement automatique des ecrans
- [ ] Etats UI systematiques : loading / empty / error + reessai / success / unauthorized / disabled
- [ ] Toasts de succes et d'erreur equivalents a `sonner`
- [ ] Theme clair / sombre coherent avec l'identite visuelle web
- [ ] Fuseau metier Indian/Antananarivo applique partout (deja fait cote dates de commande)
- [ ] Upload et telechargement de fichiers (images produit, photo de preparation, sauvegarde, exports)
- [ ] Recherche, filtres, tris et pagination equivalents sur chaque liste
...
### `/users (libellé sidebar : "Super Admin", titre page : "Super Administration")` — PARTIAL
...
### `/transfers (composant partage TransferProductsPanel — coeur fonctionnel)` — PARTIAL
...
### `/stores (modal TransferProductsDialog — variante modale du meme flux)` — PARTIAL
...
### `(global) TopBar — cloche de notifications (dropdown)` — PARTIAL
...
### `/* (RootLayout — enveloppe TOUTES les pages, y compris /login, /register, /forgot-password, /reset-password, /verify-email, /pending-approval)` — PARTIAL
...
### `/(app)/* — shell applicatif protege (couvre /dashboard, /orders, /pickup, /bilan, /products, /caisse, /chats, /movements, /alerts, /suppliers, /transfers, /notifications, /reports, /stores, /users, /settings, /sales, /superadmin)` — PARTIAL
...
### `/(app)/* — Sidebar de navigation (rendue sur toutes les routes du groupe (app))` — PARTIAL
...
### `/(app)/* — TopBar (barre superieure, rendue sur toutes les routes du groupe (app))` — PARTIAL
...
### `* — GlobalErrorBoundary (composant transverse, actuellement NON monte)` — PARTIAL
...
### `* — ThemeProvider (wrapper de theme, monte dans app/layout.tsx)` — PARTIAL
...
### `(aucune route) — components/ui/sidebar.tsx : kit de primitives shadcn/ui NON UTILISE dans l'application` — PARTIAL
...
### `(composant partagé) <ImageUpload /> — zone de dépôt + prévisualisation d'image` — PARTIAL
...
### `(composant partagé) <ProductImageGallery /> — galerie photos produit + QR codes` — PARTIAL
...
### `(composant partagé) <ConfirmDeleteDialog /> — modal de suppression avec ré-authentification par mot de passe. Monté dans /superadmin et /users` — PARTIAL
...
### `(composant partagé) <AIAnalysis /> — carte « Analyse IA Stratégique ». Monté en bas de la route /reports` — PARTIAL
...
### `POST /api/ai/analyze — route API Next.js (Route Handler)` — PARTIAL
...
### `POST /api/ai/check-duplicates — route API Next.js (Route Handler)` — PARTIAL
...
### `(module partagé) lib/image-service.ts — service de gestion des images` — PARTIAL
...
### `(module partagé) lib/qrcode-generator.ts — génération et lecture de QR codes` — PARTIAL
...
### `(hook partagé) lib/hooks/useDeliveryZones.ts — zones de livraison configurables` — PARTIAL
...
### `(hook partagé) lib/hooks/useDebouncedValue.ts — valeur temporisée pour les recherches` — PARTIAL
...
### `(couche transport) DjangoAPIClient — coeur HTTP/JWT` — PARTIAL
...
### `djangoClient.auth — authentification, inscription, mot de passe oublie` — PARTIAL
...
### `djangoClient.passwordResetRequests — moderation des demandes de reinitialisation (cote admin)` — PARTIAL
...
### `djangoClient.catalog.categories — Categories du catalogue (§8)` — PARTIAL
...
### `djangoClient.catalog.types — Sous-types rattaches a une categorie` — PARTIAL
...
### `djangoClient.catalog.brands — Marques` — PARTIAL
...
### `djangoClient.catalog.colors — Couleurs (referentiel des variantes)` — PARTIAL
...
### `djangoClient.catalog.references — References produit (fiche article) + import/export Excel` — PARTIAL
...
### `djangoClient.catalog.importBatches — Annulation d'un import Excel` — PARTIAL
...
### `djangoClient.catalog.variants — Variantes (couleur) et ajustement de stock` — PARTIAL
...
### `djangoClient.orders — Module Commandes (coeur metier, §5-§7)` — PARTIAL
...
### `djangoClient.zones — Zones de livraison configurables (Parametres)` — PARTIAL
...
### `djangoClient.movements — Historique des mouvements de stock (§7.4/§10)` — PARTIAL
...
### `djangoClient.products — Service de compatibilite (lecture seule)` — PARTIAL
...
### `djangoClient.sales — Ventes derivees des commandes livrees (compatibilite)` — PARTIAL
...
### `djangoClient.notifications — Notifications in-app` — PARTIAL
...
### `djangoClient.users — Utilisateurs, employes, profil` — PARTIAL
...
### `djangoClient.dashboard — Indicateurs generaux` — PARTIAL
...
### `djangoClient.transfers — Transfert de stock entre magasins + vue benefice` — PARTIAL
...
### `djangoClient.caisse — Sessions de caisse, mouvements, categories, synthese` — PARTIAL
...
### `djangoClient.suppliers — Commandes fournisseur (§7.6)` — PARTIAL
...
### `djangoClient.backup — Sauvegarde / restauration (admin uniquement)` — PARTIAL
...
### `djangoClient.chat — Messagerie interne` — PARTIAL
...
### `lib/types.ts — Modeles TypeScript partages` — PARTIAL
...
### `lib/validation.ts — Schemas Zod (heritage, non branches sur l'API actuelle)` — PARTIAL
...
### `lib/timezone.ts — Fuseau metier Indian/Antananarivo (regle du 'jour J')` — PARTIAL
...
### `lib/utils.ts — Helper de classes CSS` — PARTIAL
