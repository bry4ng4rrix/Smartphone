...
- **Ticket au gérant** (`services.envoyer_ticket_retour`) : quand c'est le **livreur** qui déclare un `RETOUR` ou une livraison partielle (article annulé par le client), un message privé livreur → gérant du magasin (compte `magasin`, à défaut l'admin ; salon `dm_<a>_<b>`) est créé dans la messagerie, diffusé sur `/ws/chat/` et notifié (`chat`). Contenu texte structuré : `🎫 TICKET RETOUR · Commande CMD-…` (ou `🎫 TICKET ARTICLE ANNULÉ · …`), livreur + date/heure, client + téléphones, adresse/zone, liste des articles rapportés (et livrés, en partiel) avec quantités et montants, `Motif du livreur : « … »` (la `note` saisie ; « non précisé » si vide) et une ligne `Résultat : …` (retour complet → rien livré / tout en stock / aucun encaissement ; partiel → articles remis / rapportés et montant encaissé). La photo jointe à l'étape est recopiée dans le message. Le gérant qui force lui-même un retour ne déclenche pas de ticket.
